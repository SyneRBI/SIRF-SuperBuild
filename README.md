...
```bash
source ~/devel/INSTALL/bin/env_ccpsynerbi.sh
```
You probably want to add a similar line to your .bashrc/.profile.

Or for csh
```csh
source ~/devel/INSTALL/bin/env_ccpsynerbi.csh
```
You probably want to add a similar line to your .cshrc.

Notice that for backwards compatibility a symbolic link to `env_ccpsynerbi.sh` with the name `env_ccppetmr.sh` will be created, and similarly for the csh.
