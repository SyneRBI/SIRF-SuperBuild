...
ARG BASE_IMAGE=quay.io/jupyter/scipy-notebook:ubuntu-24.04
...
RUN if test "$BUILD_GPU" != 0; then \
  sed -ri -e 's/^(\s*)#\s*(- \S+.*#.*GPU.*)$/\1\2/' -e 's/^(\s*)(- \S+.*#.*CPU.*)$/\1#\2/' /opt/scripts/requirements.yml; \
...
RUN --mount=type=cache,target=/opt/ccache BUILD_FLAGS="-G Ninja\
 ...
 -DSTIR_BUILD_EXECUTABLES:BOOL=${STIR_BUILD_EXECUTABLES}\
